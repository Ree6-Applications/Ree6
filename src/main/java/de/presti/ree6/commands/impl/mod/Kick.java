        ...
        if (commandEvent.getMember().hasPermission(Permission.KICK_MEMBERS)) {
            ...
            if (commandEvent.isSlashCommand()) {
                ...
                if (targetOption != null) {
                    ...
                    Main.getInstance().getCommandManager().sendMessage("No User was given to Kick!", 5, commandEvent.getChannel(), commandEvent.getInteractionHook());
                }
                ...
                if (commandEvent.getArguments().length <= 1 && commandEvent.getArguments().length <= 2) {
                    if (commandEvent.getMessage().getMentions().getMembers().isEmpty()) {
                        ...
                        Main.getInstance().getCommandManager().sendMessage("Use " + Main.getInstance().getSqlConnector().getSqlWorker().getSetting(commandEvent.getGuild().getId(), "chatprefix").getStringValue() + "kick @user", 5, commandEvent.getChannel(), commandEvent.getInteractionHook());
                    } else {
    ...
    public void kickMember(Member member, String reason, CommandEvent commandEvent) {
        if (commandEvent.getGuild().getSelfMember().canInteract(member) && commandEvent.getMember().canInteract(member)) {
            ...
            if (commandEvent.getGuild().getSelfMember().canInteract(member)) {
                Main.getInstance().getCommandManager().sendMessage("Couldn't kick this User because he has the same or a higher Rank then you!", 5, commandEvent.getChannel(), commandEvent.getInteractionHook());
            } else {
                Main.getInstance().getCommandManager().sendMessage("Couldn't kick this User because he has a higher Rank then me!", 5, commandEvent.getChannel(), commandEvent.getInteractionHook());
            }
        }
    }
}
