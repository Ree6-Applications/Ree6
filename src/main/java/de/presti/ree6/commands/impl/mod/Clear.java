            ...
            if(args.length == 1) {
                try {
                    if (Integer.parseInt(args[0]) <= 100 && Integer.parseInt(args[0]) >= 2) {
                        ...
                        sendMessage("Use " + Main.sqlConnector.getSqlWorker().getSetting(sender.getGuild().getId(), "chatprefix").getStringValue() + "clear 1-100", 5, m, hook);
                    }
                } catch (Exception ex) {
                    sendMessage(args[0] + " isn't a number!", 5, m, hook);
                    sendMessage("Use " + Main.sqlConnector.getSqlWorker().getSetting(sender.getGuild().getId(), "chatprefix").getStringValue() + "clear 2-100", 5, m, hook);
