...
public class Server extends Command {
    ...
    public void onPerform(Member sender, Message messageSelf, String[] args, TextChannel m) {
        if (sender.hasPermission(Permission.ADMINISTRATOR)) {
            ...
            em.addField(":busts_in_silhouette: **Members (" + m.getGuild().getMemberCount() + ")**", "**" + m.getGuild().getMembers().stream().filter(member -> !member.getOnlineStatus().equals(OnlineStatus.OFFLINE)).filter(member -> !member.getOnlineStatus().equals(OnlineStatus.UNKNOWN)).filter(member -> !member.getUser().isBot()).count() + "** Online\n**" + m.getGuild().getBoostCount() + "** Boosts :sparkles:", true);
